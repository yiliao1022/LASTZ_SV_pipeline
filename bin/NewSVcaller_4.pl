...
if (@ARGV < 1 || $Help){
print <<"END.";
  Usage: perl $0 [options] <SynNet file>
  -outdir   outdir
  -verbose  verbose
  -help     help
END.
...
my @daughters = $root->daughters; # @Chromosome 
	 foreach my $daughter (@daughters) {
             my @dayghters_playerA=$daughter->daughters; # Retrieving top-level filled segments.      
		foreach my $playerA_dau (@dayghters_playerA) {
                my $mama=$playerA_dau->mother;
				my $mamaname=$mama->name;
                my @daughters_playerB_dau=$playerA_dau->daughters;	# Gaps may represent SVs	
	            
				########################################################################
				if (@daughters_playerB_dau) { # Gaps under top filled segments
                                             
						foreach my $playerB_dau (@daughters_playerB_dau) {	# for each gap under top filled segments
			                    my $name=$playerB_dau->name;
							    my @tmp = split (/_/,$name);
							    my $Tend = $tmp[1]+$tmp[2];
							    my $Qend = $tmp[5]+$tmp[6];
							  
							    my @daughters_playerC_dau=$playerB_dau->daughters; # Filled segments under gaps may represent Duplications, Inversions ...
                                    
									if (@daughters_playerC_dau){	# if gaps exist filled segments
							                #######################Inv, Complex SVs############################
								...
								if ($tmp[6]==0) {			
											  ...
											  foreach my $playerC_dau (@daughters_playerC_dau) {	# For each filled segments
                                                    ...
                                                    if ($ali > $tmp[2]/4) {
													print OUTcnv "$mamaname\t$tmp[1]\t$Tend\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$Qend\t$array[0]\t$array[-1]\tCNV\n";
							                        } else {
							                        print OUTind "$mamaname\t$tmp[1]\t$Tend\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$Qend\tInsertion\n";
							                        }
											  ...
											  foreach my $playerC_dau (@daughters_playerC_dau) {	# For each filled segments
										              ...
										              if ($$hash{"type"} =~/inv/ and $$hash{"qFar"} < $tmp[2] and $$hash{"ali"}>$tmp[2]/2 and $tmp[6]/$tmp[2] > 0.80){ # Define inversions
														  
														  print OUTinv "$mamaname\t$tmp[1]\t$Tend\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$Qend\tInversion_min\n";
														  last;
														  
														} elsif ($$hash{"type"}=~/syn|inv/ and  $$hash{"qFar"} < 2*$tmp[2] ) { # Define copy number variations
												...
												if ($ali > $tmp[2]/4) {
												print OUTcnv "$mamaname\t$tmp[1]\t$Tend\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$Qend\t$array[0]\t$array[-1]\tCNV_min\n";
												} else {
												print OUTcxs "$mamaname\t$tmp[1]\t$Tend\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$Qend\tComplex_min\n";
                                                }
				                    ...
				                    } else { # if gaps do not exist any filled segments, e.g. Completely absence of homologous sequences in the query genome
				                          
											    if ($tmp[6]==0) {
 				                                  print OUTind "$mamaname\t$tmp[1]\t$Tend\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$Qend\tInsertion_PAV\n"; # Here only record the Inserion to reference
												  $PAV = $PAV + $tmp[2];
											    } else {
											      print OUTcxs "$mamaname\t$tmp[1]\t$Tend\t$tmp[3]\t$tmp[4]\t$tmp[5]\t$Qend\tComplex_PAV\n"; # Here indicate potential convergent signals in evolution
												     if ($tmp[2]/$tmp[6] > 4) {
												     $PAV = $PAV + $tmp[2]-$tmp[6];
													 }
											    }

...
if ($Help){
print <<"END.";
  Usage: perl $0 [options] -SynNetFile <SynNet file>
  -outdir   outdir
  -SynNet <SynNet file>
  ...
  -help     help
    ...
    Target start
    target end
    SV type (internal is contained within an alignment block)
    SV length (insertions are in query length)
    Percent identity in alignment block or for flanking alignment blocks
    Number of bases matching in alignment block or for flanking alignment blocks
    Query seqid
    Query start
    Query end
    Sequence (for deletions it's the target sequence and insertions are the query sequence)
							    ...
							    my @daughters_playerC_dau=$daughters_playerB_dau[$i]->daughters; # The segments for filling the gaps.
								    ...
								    if ($tmp[6] < 10) {														
											  ...
											  foreach my $playerC_dau (@daughters_playerC_dau) {	# For each filled segments
										            ...
										            #my @array1 = sort {$a <=> $b} @len;
										            
                                                    if ($ali/$tmp[2] > $ali_rate) {
											  ...
											  foreach my $playerC_dau (@daughters_playerC_dau) { # For each filled segments
												...
												if (($inv_len > ($tmp[6]+$tmp[2])/4 or $inv_len>4000) and 0.25 < $tmp[6]/$tmp[2] and $tmp[6]/$tmp[2]< 4) {
